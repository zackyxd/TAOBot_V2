...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('make-l2w-clan')
    .setDescription("Set L2W clans for nudges")
    .addStringOption(option =>
      option.setName("abbreviations")
        .setDescription("Abbreviation(s) of clans, separated by space")
        .setRequired(true)
    )
  ...
  async execute(interaction) {
    ...
    if (interaction.commandName !== "make-l2w-clan") return;
    ...
    for (const [clantag, clanInfo] of Object.entries(clans)) {
      ...
      // console.log(clantag);
      await db.set(`clans.${clantag}.l2w`, true);
      createdList.push(clanInfo.abbreviation);
    }

    await interaction.editReply({ embeds: [createSuccessEmbed(`Made ${createdList.join(',')} into throw clans`)] });
